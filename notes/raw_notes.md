...
##### Longer, error checking version

$ProgressPreference = 'SilentlyContinue'  # turn off progress box. greatly improves performance.

If(test-path "C:\Forensic_Program_Files\Windows_Artifacts\Woanware\" -filter "autorunner*" ) { Get-childitem "C:\Forensic_Program_Files\Windows_Artifacts\Woanware\" -filter "autorunner*"| Remove-Item -Force -Recurse }

$download_url   = "https://github.com/woanware/autorunner/releases/download/v0.0.16/autorunner.v0.0.16.zip"

$filename   	= [System.IO.Path]::GetFileName($download_url)  # parse out the file name from the download url  

$basename 		= [System.IO.Path]::GetFileNameWithoutExtension($download_url)

Invoke-WebRequest -Uri $download_url  -OutFile "C:\Installer_apps\$filename"   # download the app installer 

Expand-Archive -Path "C:\installer_apps\$filename" -DestinationPath "C:\Forensic_Program_Files\Windows_Artifacts\Woanware\$basename" -Force

# Create a subdir named tools under the autorunner folder and copy Sysinternals sigcheck.exe into that new folder.  Required for autorunner to run.
If(!(test-path "C:\Forensic_Program_Files\Windows_Artifacts\Woanware\$basename\Tools")) { New-Item -Path "C:\Forensic_Program_Files\Windows_Artifacts\Woanware\$basename\Tools" -ItemType Directory -Force }

Copy-Item -Path "C:\Forensic_Program_Files\SysinternalsSuite\sigcheck.exe" -Destination "C:\Forensic_Program_Files\Windows_Artifacts\Woanware\$basename\Tools"  -Force
